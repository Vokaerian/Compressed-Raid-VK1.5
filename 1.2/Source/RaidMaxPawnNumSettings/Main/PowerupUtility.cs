...
namespace CompressedRaid
{
    public class PowerupUtility
    {
        ...
        private static IEnumerable<PawnCapacityModifier> GetPCMList(float gainStatValue)
        {
            ...
            if (CompressedRaidMod.gainFactorMultCapacityMetabolismValue > 0f && CompressedRaidMod.gainFactorMaxCapacityMetabolismValue > 0f)
            {
                yield return new PawnCapacityModifier() { capacity = PawnCapacityDefOf.Metabolism, offset = Math.Min(gainStatValue * CompressedRaidMod.gainFactorMultCapacityMetabolismValue, CompressedRaidMod.gainFactorMaxCapacityMetabolismValue) };
            }
