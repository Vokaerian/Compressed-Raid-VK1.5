...
namespace CompressedRaid
{
    ...
    public class DrugHediffDataStore
    {
        ...
        //private static int TECH_LEVEL_INT_MAX = 0;
            ...
            while (loopMax > loopCount && addDrugMaxNumberValue > addDrugNum && drugs.Any() && !forceEnd)
            {
                ...
                if (!hasHediff)
                {
                    ...
                    else
                    {
                        ...
                        if (drug.drug == ThingDefOf.Luciferium)
                        {
                            Log.Message(String.Format("CR_MESSAGE: {0}は常に通常付与！", drug.drug.LabelCap));
                        }
                        else
                        {
                            Log.Message(String.Format("CR_MESSAGE: {0}は設定に従い通常付与！", drug.drug.LabelCap));
                        }
                        //test
#endif
                        if (!forceEnd)
